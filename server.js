...
const checkGuess = (answer, guess) => {
  let feedback = [];
  for (let i = 0; i < guess.length; i++) { // we want to first find all the correct nums and change them to -1s to eliminate them.
    if (guess[i] === answer[i]) {
  ...
  for (let j = 0; j < answer.length; j++) { // Need to a loop to check the rest of the answers
    ...
    for (let k = 0; k < guess.length; k++) { // use a second loop to iterate over the guess
      if (guess[k] === -1) { // make sure we skip -1s in the guess
        continue;
      } else if (answer[j] === guess[k]) { // if an elemnt in the guess equals to one in the answer, this means we hit a partial
